...
This page provides procedures and guidelines for developing and contributing to sasquatch.

Scope of contributions
======================

sasquatch is an open source package, meaning that you can contribute to sasquatch itself, or fork sasquatch for your own purposes.

Since sasquatch is intended for internal use by Rubin Observatory, community contributions can only be accepted if they align with Rubin Observatory's aims.
For that reason, it's a good idea to propose changes with a new `GitHub issue`_ before investing time in making a pull request.

sasquatch is developed by the Rubin Observatory SQuaRE team.
...
To develop sasquatch, create a virtual environment with your method of choice (like virtualenvwrapper) and then clone or fork, and install:
...
1. Installs sasquatch in an editable mode with its "dev" extra that includes test and documentation dependencies.
...
If the next version is known (because sasquatch's main branch is being prepared for a new major or minor version), the section may contain that version information:
